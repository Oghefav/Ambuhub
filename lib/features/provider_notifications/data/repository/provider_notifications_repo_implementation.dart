...
import 'package:ambuhub/features/provider_notifications/data/model/provider_notification_model.dart';
...
class ProviderNotificationsRepoImplementation
    implements ProviderNotificationsRepo {
  final ProviderNotificationsApiService _apiService;

  const ProviderNotificationsRepoImplementation(this._apiService);

  bool _isSuccess(int? code) =>
      code == 200 || code == 201 || code == 204;

  List<ProviderNotificationEntity> _parseList(dynamic raw) {
    return parseNotificationList(raw)
        .map(ProviderNotificationModel.fromJson)
        .toList();
  }

  ProviderNotificationEntity _parseOne(dynamic raw) {
    if (raw is Map<String, dynamic>) {
      final nested = raw['notification'];
      if (nested is Map<String, dynamic>) {
        return ProviderNotificationModel.fromJson(nested);
      }
      return ProviderNotificationModel.fromJson(raw);
    }
    throw const FormatException('Invalid notification response');
  }

  @override
  Future<DataState<List<ProviderNotificationEntity>>> getNotifications() async {
    try {
